...
class DirectoryScanner:
    ...
    def _validate_directory_path(self, directory_path: str) -> Path:
        ...
        try:
            ...
            raise InvalidDirectoryPathError(
                f"Cannot access directory: {directory_path} - {str(e)}"
            )
    ...
    def _find_pdf_files(self, directory_path: Path) -> List[Path]:
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(unique_files)} PDF files in {directory_path}"
            )
            return unique_files
    ...
    def _extract_job_info(self, directory_path: Path) -> Tuple[str, str]:
        ...
        self.logger.debug(
            f"Extracted job info: name='{job_name}', number='{job_number}'"
        )
    ...
    def _get_file_info(self, pdf_file: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(
                f"Could not get complete file stats for {pdf_file}: {e}"
            )
            return {
                "filename": pdf_file.name,
                "file_path": str(pdf_file),
    ...
    def _analyze_filename(self, filename: str) -> Dict[str, Any]:
        ...
        hints["contract_indicators"] = [
            ind for ind in main_indicators if ind in filename_lower
        ]
        hints["likely_main_contract"] = len(hints["contract_indicators"]) > 0

        # Execution status indicators
        execution_indicators = ["executed", "signed", "fully executed"]
        hints["status_indicators"] = [
            ind for ind in execution_indicators if ind in filename_lower
        ]
